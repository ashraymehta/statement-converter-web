...
export class StandardCharteredAdapter extends TransactionAdapter {
    public async convert(xlsxData: ArrayBuffer): Promise<Transaction[]> {
        ...
        const addressForTransactionDate = XLSXUtil.findTextIgnoringWhitespace(sheet, 'Date');
        const addressForDetails = XLSXUtil.findTextIgnoringWhitespace(sheet, 'Transaction');
        const addressForDepositAmount = XLSXUtil.findTextIgnoringWhitespace(sheet, 'Deposit');
        const addressForWithdrawalAmount = XLSXUtil.findTextIgnoringWhitespace(sheet, 'Withdrawal');
